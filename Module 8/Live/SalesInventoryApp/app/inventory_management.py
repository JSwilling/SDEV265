...
class ManageInventoryScreen:
    ...
    def submit_order_to_db(self, entry_fields):
        try:
            ...
            cursor.execute("SELECT stockLevel FROM InventoryItem WHERE item_id = ?", (item_id,))
            result = cursor.fetchone()
            if not result or result[0] < quantity:
                raise Exception("Insufficient stock for the order.")

            # Update stock level
            new_stock = result[0] - quantity
            cursor.execute("UPDATE InventoryItem SET stockLevel = ? WHERE item_id = ?", (new_stock, item_id))
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
            order_date, customer_name, item_id, product_name, quantity, price, delivery_address, contact_info, notes,
            agent_id))
    ...
    def submit_item_to_db(self, entry_fields):
        ...
        try:
            ...
            cursor.execute("SELECT stockLevel FROM InventoryItem WHERE SKU = ?", (sku,))
            result = cursor.fetchone()

            if result:
                # Update the existing stockLevel
                existing_stock = result[0]
                new_stock = existing_stock + stock_level
                cursor.execute("UPDATE InventoryItem SET stockLevel = ? WHERE SKU = ?", (new_stock, sku))
            else:
                # Insert a new item
                cursor.execute("INSERT INTO InventoryItem (Name, SKU, price, stockLevel) VALUES (?, ?, ?, ?)",
    ...
    def add_sale(self):
        ...
        try:
            ...
            result = cursor.fetchone()
            if not result or result[0] < quantity:
                raise Exception("Insufficient stock for the sale.")
...
class ManageStaffScreen:
    ...
    def show_employee_profile(self, event=None):
        ...
        if selected_item:
            ...
            customtkinter.CTkLabel(
                profile_window, text=f"Recent Activity: {username}", font=("Arial", 18, "bold")
            ).pack(pady=10)

            activity_frame = customtkinter.CTkFrame(profile_window)
            ...
            for action, timestamp in activities:
                customtkinter.CTkLabel(activity_frame, text=f"{timestamp}: {action}", font=("Arial", 14)).pack(
                    anchor="w")

            # Clock In/Out Calendar
            calendar_button = customtkinter.CTkButton(
                profile_window, text="View Clock In/Out Times", command=lambda: self.show_calendar(username)
            )
            calendar_button.pack(pady=10)

    def show_calendar(self, username):
        calendar_window = customtkinter.CTkToplevel(self.app)
        calendar_window.title("Clock In/Out Times")
        ...
        conn = sqlite3.connect(r'db/Sales_Inventory.db')
        cursor = conn.cursor()
        cursor.execute("SELECT clock_in, clock_out FROM clock_times WHERE username = ?", (username,))
        records = cursor.fetchall()
        conn.close()

        for clock_in, clock_out in records:
            customtkinter.CTkLabel(
                calendar_window, text=f"In: {clock_in}, Out: {clock_out}", font=("Arial", 12)
    ...
    def setup_manage_tab(self):
        ...
        # Change Role Button
        change_role_button = customtkinter.CTkButton(
            self.manage_tab, text="Change Role", command=self.change_role, fg_color="#3498DB", hover_color="#2980B9", text_color="#ffffff"
        )
        change_role_button.pack(pady=10)
    ...
    def change_role(self):
        ...
        if selected_item:
            employee_data = self.employee_table.item(selected_item, 'values')
            username = employee_data[0]
            current_role = employee_data[1]

            # Create a popup window for changing the role
            change_role_window = customtkinter.CTkToplevel(self.app)
            change_role_window.title("Change Role")
            change_role_window.geometry("400x300")

            # Set the window to always be on top
            change_role_window.attributes('-topmost', True)

            customtkinter.CTkLabel(
                change_role_window,
                text=f"Change Role for {username}",
                font=("Arial", 18, "bold"),
                text_color="#34495e"
            ).pack(pady=20)

            role_var = tk.StringVar(value=current_role)
            roles = ["Employee", "Supervisor", "Salesperson"]  # Define allowed roles
            ...
            def update_role():
                ...
                if new_role and new_role != current_role:
                    try:
                        # Update the database
                        connection = sqlite3.connect(r'db/Sales_Inventory.db')
                        cursor = connection.cursor()
                        ...
                        self.employee_table.item(selected_item, values=(username, new_role))
                        messagebox.showinfo("Success", f"Role updated to {new_role} for {username}.")
                        change_role_window.destroy()
                    except Exception as e:
                        messagebox.showerror("Error", f"Failed to update role: {e}")
                else:
                    messagebox.showwarning("No Change", "Role was not updated.")

            update_button = customtkinter.CTkButton(
                change_role_window, text="Update Role", command=update_role, fg_color="#2ECC71", hover_color="#27AE60",
                text_color="#ffffff"
            )
